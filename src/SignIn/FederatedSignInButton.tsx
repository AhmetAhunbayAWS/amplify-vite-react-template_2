...
export interface FederatedSignInButtonProps {
  icon?: SocialProvider | React.ReactNode;
  provider: FederatedIdentityProviders | string;
  text: string;
  clickHandler: (provider: FederatedIdentityProviders | string) => (event: React.MouseEvent<HTMLButtonElement>) => void;
}
...
export const FederatedSignInButton = (
  ...
  const { icon, provider, text, clickHandler } = props;
